(() => {
  ...
  const form = document.querySelector('#steep-inquiry-form');
  const preferredConcept = form?.querySelector('input[name="preferred_concept"]');
  ...
  const photoOutput = form?.querySelector('.steep-upload output');
  const formError = form?.querySelector('.steep-form__error');
  const formStatus = form?.querySelector('.steep-form__status');
  ...
  function selectPath(card) {
    ...
    if (choice) {
      ...
      choice.querySelector('a').href = '#steildach-anfrage';
    }
    if (preferredConcept) preferredConcept.value = selected.title;
    if (requestSelection) requestSelection.textContent = selected.title;
  }

  cards.forEach((card) => card.querySelector('button')?.addEventListener('click', () => selectPath(card)));
  ...
  form?.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      heading.textContent = 'Ihre Steildach-Anfrage ist eingegangen';
